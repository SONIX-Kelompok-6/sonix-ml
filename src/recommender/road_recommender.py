...
road running shoe features. Implements data-driven feature mapping and 
...
def preprocess_road_input(user_input: Dict[str, Any], 
    ...
    Applies heuristic mapping to convert qualitative preferences into quantitative 
    features. Utilizes dictionary lookups and set operations to eliminate branching 
    and maintain O(1) cyclomatic complexity where possible.
    ...
    # Direct dictionary mapping replacing nested ternary operators
    ...
    feats['midsole_softness'] = get_priority_val(user_input, prio_soft, {'cushion_preferences': {'Soft': 1.0, 'Balanced': 0.6, 'Firm': 0.2}, 'pace': {'Easy': 1.0, 'Steady': 0.6, 'Fast': 0.2}})

    prio_width = ['stability_need', 'foot_width']
    feats['width_fit'] = get_priority_val(user_input, prio_width, {'stability_need': {'Neutral': 0.5, 'Guided': 0.2}, 'foot_width': {'Narrow': 0.2, 'Regular': 0.6, 'Wide': 1}})
    ...
    feats['stiffness_scaled'] = get_priority_val(user_input, prio_stiff, {'arch_type': {'Flat': 0.0, 'Normal': 0.5, 'High': 0.5}, 'pace': {'Easy': 0.2, 'Steady': 0.6, 'Fast': 1.0}, 'running_purpose': {'Daily': 0.2, 'Tempo': 0.6, 'Race': 1}})
    ...
    feats['plate_carbon_plate'] = get_priority_val(user_input, prio_plate, {'pace': {'Easy': 0.5, 'Steady': 0.5, 'Fast': 1.0}, 'running_purpose': {'Daily': 0.5, 'Tempo': 0.5, 'Race': 1.0}})

    prio_stack = ['strike_pattern', 'pace', 'running_purpose']
    feats['heel_lab_mm'] = get_priority_val(user_input, prio_stack, {'strike_pattern': {'Heel': 1.0, 'Mid': 0.5, 'Forefoot': 0.0}, 'pace': {'Easy': 1.0, 'Steady': 0.5, 'Fast': 0.0}})
    feats['forefoot_lab_mm'] = get_priority_val(user_input, prio_stack, {'strike_pattern': {'Heel': 0.0, 'Mid': 0.5, 'Forefoot': 1.0}, 'pace': {'Easy': 0.0, 'Steady': 0.5, 'Fast': 1.0}})
    ...
    # Set intersection replaces 'any' loop for feature masking
...
def get_recommendations(user_input: Dict[str, Any], artifacts: Dict[str, Any]) -> List[str]:
    ...
    Returns:
        List[str]: A list containing the top recommended road shoe IDs.
    """
